...
def add_user_to_leaderboard(username, password_hash, display_name, color):
    ...
    with get_db_connection() as db:
        cur = db.execute(
            ...
            (username, hashed_pw, display_name, color)
        )
...
def update_kills(user_id, kills=1):
    with get_db_connection() as db:
        db.execute(
            'UPDATE leaderboard SET kills = kills + ? WHERE user_id = ?',
            (kills, user_id)
        )
        db.commit()
...
def update_deaths(user_id, deaths=1):
    with get_db_connection() as db:
        db.execute(
            'UPDATE leaderboard SET deaths = deaths + ? WHERE user_id = ?',
            (deaths, user_id)
        )
        db.commit()
...
def get_leaderboard(limit=10):
    with get_db_connection() as db:
        return db.execute(
            'SELECT u.username, l.kills, l.score, l.matches_played '
            'FROM leaderboard l JOIN users u ON l.user_id = u.id '
            'ORDER BY l.kills DESC, l.score DESC LIMIT ?',
            (limit,)
        ).fetchall()
